...
public class POIWord2007Utils {
	 ...
	 */
	public void simpleNumberFooter(XWPFDocument document, String fontFamily, String fontSize) throws Exception {
		CTP ctp = CTP.Factory.newInstance();
		XWPFParagraph codePara = new XWPFParagraph(ctp, document);
		XWPFRun r1 = codePara.createRun();
		r1.setText("第");
		setParagraphRunFontInfo(codePara, r1, null, fontFamily, fontSize);
	 ...
	 */
	public void simpleDateHeader(XWPFDocument document, String fontFamily, String fontSize) throws Exception {
		CTP ctp = CTP.Factory.newInstance();
		XWPFParagraph codePara = new XWPFParagraph(ctp, document);
	...
	public static void main(String[] args) throws Exception {
		...
		t.addNewPageBreak(xdoc, BreakType.TEXT_WRAPPING);
		String filePath = "f:/saveFile/temp/sys_" + System.currentTimeMillis() + ".docx";
		...
		XWPFDocument xdoc3 = t.openDocument("f:/saveFile/temp/sys_1423892350239.docx");
		XWPFTable table2 = t.getTableByIndex(xdoc2, 0);
		List<List<String>> tableList2 = t.getTableRContent(table);
		for (List<String> list : tableList2) {
			System.out.println(list);
		}
		t.insertTableRowAtIndex(table2, 3);
		t.deleteTableRow(table2, 4);
		t.deleteTableRow(table2, 3);
		t.setCellNewContent(table2, 0, 0, "新内容");
		t.deleteCellContent(table2, 1, 0);
		t.setHiddenCellContent(table2, 2, 1);
		t.deleteTableByIndex(xdoc3, 1);
		t.saveDocument(xdoc3, "f:/saveFile/temp/sys_" + System.currentTimeMillis() + ".docx");
	}
	...
	public void testFillTableValue(XWPFTable table) {
		for (Integer rowIndex = 0, rowLen = table.getNumberOfRows(); rowIndex < rowLen; rowIndex++) {
			XWPFTableRow row = table.getRow(rowIndex);
			for (Integer colIndex = 0; colIndex < row.getTableCells().size(); colIndex++) {
				if (rowIndex == 0) {
					XWPFTableCell cell = row.getCell(colIndex);
					setCellVAlign(cell, STVerticalJc.TOP);
					XWPFParagraph p = getCellFirstParagraph(cell);
					XWPFRun pRun = getOrAddParagraphFirstRun(p, false, false);
					Integer value = rowIndex - 1;
					System.out.println("第" + rowIndex + "行-------------------》" + value);
					setParagraphRunFontInfo(p, pRun, "", "微软雅黑", "3");
					// setParagraphTextBasicStyleInfo(p, pRun, null, false,
					// false, null, null, true, false, false, null,
					// true, STShd.HORZ_STRIPE, "F3C917");
					...
					Integer value = rowIndex - 1;
					System.out.println("第" + rowIndex + "行-------------------》" + value);
					setParagraphRunFontInfo(p, pRun, "0123456789", "微软雅黑", "8");
					// setParagraphTextBasicStyleInfo(p, pRun, null, false,
					// false, null, null, true, false, false, null,
					// true, STShd.HORZ_STRIPE, "F3C917");
					setParagraphAlignInfo(p, ParagraphAlignment.CENTER, null);
	...
	public void testAddHeaderFooter(XWPFDocument doc) throws Exception {
		simpleDateHeader(doc, "微软雅黑", "22");
		simpleNumberFooter(doc, "微软雅黑", "22");
	}
	...
	public void testCreateParagraphWithBookMark(XWPFDocument xdoc) {
		...
		addParagraphContentBookmarkBasicStyle(p, "测试", markId, "书签一", false, false, "微软雅黑", "24", null, false, false,
				null, null, false, false);
		markId = markId.add(BigInteger.ONE);
		addParagraphContentBookmarkBasicStyle(p, "书签", markId, "书签二", true, false, "微软雅黑", "24", null, false, false,
				null, null, false, false);
	}
	...
	public void testCreateParagraphWithHyLink(XWPFDocument xdoc) {
		...
		addParagraphTextHyperlinkBasicStyle(p, "mailto:123@qq.com?subject=测试poi超链接", "超链接", "黑体", "22", "0000FF", true,
				true, false);
	...
	public void testCreateSimpleParagraph(XWPFDocument xdoc) {
		...
		setParagraphBorders(p, Borders.NONE, Borders.NONE, Borders.NONE, Borders.SINGLE, null);
		XWPFRun pRun = getOrAddParagraphFirstRun(p, false, false);
		setParagraphTextSimpleStyleInfo(p, pRun, null, true, true, "FFA932", STUnderline.DASH, true, false, true,
		...
		setParagraphTextBasicStyleInfo(p, pRun, null, false, false, null, null, true, false, false, null, true,
				STShd.HORZ_STRIPE, "F3C917");
		pRun = getOrAddParagraphFirstRun(p, true, false);
		setParagraphRunFontInfo(p, pRun, "测", "宋体", "22");
		setParagraphTextSimpleStyleInfo(p, pRun, null, false, false, null, null, true, false, false, null, false, null,
				null, null, 6, 80, 0);
		pRun = getOrAddParagraphFirstRun(p, true, false);
		setParagraphRunFontInfo(p, pRun, "上标", "宋体", "22");
		setParagraphTextSimpleStyleInfo(p, pRun, null, false, false, null, null, true, false, false, null, false, null,
				null, VerticalAlign.SUPERSCRIPT, 0, 0, 0);
		pRun = getOrAddParagraphFirstRun(p, true, false);
		setParagraphRunFontInfo(p, pRun, "下标", "宋体", "22");
		setParagraphTextSimpleStyleInfo(p, pRun, null, false, false, null, null, true, false, false, null, false, null,
				null, VerticalAlign.SUBSCRIPT, 0, 0, 0);
		pRun = getOrAddParagraphFirstRun(p, true, false);
		setParagraphRunFontInfo(p, pRun, "继续添加文本把段落内容挤下去", "宋体", "22");
		setParagraphTextBasicStyleInfo(p, pRun, null, false, false, null, null, true, false, false, null, false, null,
				null);
		...
		setParagraphSpacingInfo(p, true, "0", "0", "200", "200", true, "240", STLineSpacingRule.AUTO);
	}
	...
	public void testCreateParagraphWithImg(CustomXWPFDocument_S_9 xdoc) throws Exception {
		...
		String blipId = p.getDocument().addPictureData(new FileInputStream(new File("f:/saveFile/temp/image1.png")),
				Document.PICTURE_TYPE_PNG);
		xdoc.createPicture(blipId, xdoc.getNextPicNameNumber(Document.PICTURE_TYPE_PNG), 90, 93, p);
		pRun = getOrAddParagraphFirstRun(p, true, false);
		setParagraphRunFontInfo(p, pRun, StringUtils.leftPad("B.", 10), "宋体", "22");
		blipId = p.getDocument().addPictureData(new FileInputStream(new File("f:/saveFile/temp/image2.png")),
				Document.PICTURE_TYPE_PNG);
		xdoc.createPicture(blipId, xdoc.getNextPicNameNumber(Document.PICTURE_TYPE_PNG), 90, 93, p);
		pRun = getOrAddParagraphFirstRun(p, true, true);
		setParagraphRunFontInfo(p, pRun, "C", "宋体", "22");
		blipId = p.getDocument().addPictureData(new FileInputStream(new File("f:/saveFile/temp/image3.png")),
				Document.PICTURE_TYPE_PNG);
		xdoc.createPicture(blipId, xdoc.getNextPicNameNumber(Document.PICTURE_TYPE_PNG), 90, 93, p);
		pRun = getOrAddParagraphFirstRun(p, true, false);
		setParagraphRunFontInfo(p, pRun, StringUtils.leftPad("D.", 10), "宋体", "22");
		blipId = p.getDocument().addPictureData(new FileInputStream(new File("f:/saveFile/temp/image4.png")),
				Document.PICTURE_TYPE_PNG);
		xdoc.createPicture(blipId, xdoc.getNextPicNameNumber(Document.PICTURE_TYPE_PNG), 90, 93, p);
	}
